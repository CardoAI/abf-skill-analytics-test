...
Every answer that uses stratification / analytics data returned by an MCP tool MUST include and display the source URL — the frontend url embedded in the get_stratification_analytics_data response. No exceptions: cite and display the url for each analytic whose data informs the answer, so the analyst can open the source in the platform. If the data came from MCP strats and no url is surfaced, say so rather than omitting the citation silently.
